CREATE TABLE lk_observation_clean AS
-- rule 1, insurance
...
INSERT INTO lk_observation_clean
SELECT src.subject_id                         AS subject_id,
       src.hadm_id                            AS hadm_id,
...
CREATE TABLE lk_obs_admissions_concept AS
SELECT DISTINCT src.value_as_string      AS source_code,
                src.source_vocabulary_id AS source_vocabulary_id,
                vc.domain_id             AS source_domain_id,
                vc.concept_id            AS source_concept_id,
                vc2.domain_id            AS target_domain_id,
                vc2.concept_id           AS target_concept_id
FROM lk_observation_clean src
         LEFT JOIN
     voc_concept vc
...
CREATE TABLE lk_observation_mapped AS
SELECT src.hadm_id                        AS hadm_id,          -- to visit
